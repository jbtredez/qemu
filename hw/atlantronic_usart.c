...
static void atlantronic_usart_write(void *opaque, target_phys_addr_t offset, uint64_t val, unsigned size)
{
	...
	switch(offset)
	{
		...
		case offsetof(USART_TypeDef, GTPR):
			...
			break;
		default:
			printf("Error : USART forbiden write acces offset %x, val %lx\n", offset, val);
			break;
	}
}

static uint64_t atlantronic_usart_read(void *opaque, target_phys_addr_t offset, unsigned size)
{
	uint64_t res = 0;
	struct atlantronic_usart_state* s = opaque;
	...
	switch(offset)
	{
		...
		case offsetof(USART_TypeDef, GTPR):
			...
			break;
		default:
			printf("Error : USART forbiden read acces offset %x\n", offset);
			break;
	}
...
{
	struct atlantronic_usart_state *s = FROM_SYSBUS(struct atlantronic_usart_state, dev);

	memory_region_init_io(&s->iomem, &atlantronic_usart_ops, s, "atlantronic_usart", 0x400);
	sysbus_init_mmio(dev, &s->iomem);

	qdev_init_gpio_out(&dev->qdev, s->irq, ATLANTRONIC_USART_IRQ_MAX);
	qdev_init_gpio_in(&dev->qdev, atlantronic_usart_in_recv, 1);
