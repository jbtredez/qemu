...
{
	struct atlantronic_dma_chan_state *s = OBJECT_CHECK(struct atlantronic_dma_chan_state, dev, "atlantronic-dma-chan");

	memory_region_init_io(&s->iomem, OBJECT(s), &atlantronic_dma_chan_ops, s, "atlantronic_dma_chan", 0x010);
	sysbus_init_mmio(dev, &s->iomem);

	qdev_init_gpio_out(DEVICE(dev), &s->irq, 1);
	qdev_init_gpio_in(DEVICE(dev), atlantronic_dma_chan_in_recv, 1);
