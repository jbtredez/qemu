...
static int atlantronic_dma_init(SysBusDevice * dev)
{
	...
	memory_region_init_io(&s->iomem, OBJECT(s), &atlantronic_dma_ops, s, "atlantronic_dma", 0x08);
	sysbus_init_mmio(dev, &s->iomem);

	qdev_init_gpio_out(DEVICE(dev), s->irq, DMA_CHAN_NUMBER);
	qdev_init_gpio_in(DEVICE(dev), atlantronic_dma_in_recv, DMA_CHAN_NUMBER);
