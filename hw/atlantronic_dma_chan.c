...
static void atlantronic_dma_chan_write(void *opaque, target_phys_addr_t offset, uint64_t val, unsigned size)
{
	...
	switch(offset)
	{
		...
		case offsetof(DMA_Channel_TypeDef, CMAR):
			...
			break;
		default:
			printf("Error : DMA Channel forbiden write acces offset %x, val %lx\n", offset, val);
			break;
	}
}

static uint64_t atlantronic_dma_chan_read(void *opaque, target_phys_addr_t offset, unsigned size)
{
	uint64_t res = 0;
	struct atlantronic_dma_chan_state* s = opaque;
	...
	switch(offset)
	{
		...
		case offsetof(DMA_Channel_TypeDef, CMAR):
			...
			break;
		default:
			printf("Error : DMA Channel forbiden read acces offset %x\n", offset);
			break;
	}
