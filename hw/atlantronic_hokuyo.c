...
{
	struct atlantronic_hokuyo_state *s = FROM_SYSBUS(struct atlantronic_hokuyo_state, dev);

	sysbus_init_mmio(dev, &s->iomem);

	qdev_init_gpio_out(&dev->qdev, &s->irq, 1);
	qdev_init_gpio_in(&dev->qdev, atlantronic_hokuyo_in_recv, HOKUYO_IRQ_IN_NUM);
