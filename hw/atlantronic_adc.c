...
static void atlantronic_adc_write(void *opaque, target_phys_addr_t offset, uint64_t val, unsigned size)
{
	...
	switch(offset)
	{
		...
		default:
			printf("Error : ADC forbiden write acces offset %x, val %lx\n", offset, val);
			break;
	}
}

static uint64_t atlantronic_adc_read(void *opaque, target_phys_addr_t offset, unsigned size)
{
	uint64_t res = 0;
	struct atlantronic_adc_state* s = opaque;
	...
	switch(offset)
	{
		...
		case offsetof(ADC_TypeDef, DR):
			...
			break;
		default:
			printf("Error : ADC forbiden read acces offset %x\n", offset);
			break;
	}
...
{
	struct atlantronic_adc_state *s = FROM_SYSBUS(struct atlantronic_adc_state, dev);

	memory_region_init_io(&s->iomem, &atlantronic_adc_ops, s, "atlantronic_adc", 0x400);
	sysbus_init_mmio(dev, &s->iomem);

	qdev_init_gpio_out(&dev->qdev, &s->irq, 1);
	qdev_init_gpio_in(&dev->qdev, atlantronic_adc_in_recv, AN_NUM);
